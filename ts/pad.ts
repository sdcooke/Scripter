...
var pressedNotes: Map<Pitch, boolean> = new Map();
...
var sustainedNotes: Map<Pitch, boolean> = new Map();
...
export const plugin: ScripterPlugin = {
    ...
    HandleMIDI: (event: MidiEvent) => {
        ...
        if (event instanceof ControlChange && event.number === CC.Hold) {
            sustainPedalHeld = !!event.value;
            if (!sustainPedalHeld) {
                sustainedNotes.forEach((enabled, pitch) => {
    ...
    ParameterChanged: (param: number, value: number) => {
        ...
        if (wasDisabled && !newDisabled && param == 2) {
            ...
            pressedNotes.forEach((enabled, pitch) => {
                if (!enabled) {
                    const noteOn = new NoteOn();
                    noteOn.pitch = pitch;
                    noteOn.send();
